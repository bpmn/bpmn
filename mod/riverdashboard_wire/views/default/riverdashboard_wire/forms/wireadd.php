...
function textCounter(field,cntfield,maxlimit) {
	...
	<form action="<?php echo $vars['url']; ?>action/riverdashboard/wireadd" method="post" name="noteForm">
			<?php
			    $display .= "<input type='text' name='note' value='' onKeyDown=\"textCounter(document.noteForm.note,document.noteForm.remLen1,140)\" onKeyUp=\"textCounter(document.noteForm.note,document.noteForm.remLen1,140)\" style='width:500px' maxlength='140'>{$msg}</input>";
			?>
			<input type="hidden" name="method" value="site" />
			<input type="submit" style="float:right; margin-right:30px;" value="<?php echo elgg_echo('thewire:post'); ?>" /></td></tr></table>
			<?php
