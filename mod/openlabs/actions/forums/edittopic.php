<?php
    ...
    /**
	 * Elgg openlabs plugin edit topic action.
	 * 
...
if (!$openlab_entity->isMember($vars['user']) && !isadminloggedin()) {
	forward();
}
		...
		if ($topic)
		{
			...
			// Make sure the title isn't blank
				if (empty($title) || empty($message)) {
					register_error(elgg_echo("openlabstopic:blank"));
	        ...
	        // now let's edit the message annotation
				   update_annotation($message_id, "openlab_topic_post", $message, "",$user, $access);
			...
			// save the changes
	           ...
	           // Success message
					system_message(elgg_echo("openlabs:forumtopic:edited"));
