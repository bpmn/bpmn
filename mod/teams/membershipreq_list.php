<?php
	...
	require_once(dirname(dirname(dirname(__FILE__))) . "/engine/start.php");
        require_once(dirname(__FILE__)."/lib/team_lib.php") ;
	gatekeeper();
	
	$username = get_input('username');
        $user=get_user_by_username($username);
	$list_reqs=list_joinrequest_teams($user);
		...
		//$requests = elgg_get_entities_from_relationship(array('relationship' => 'membership_request', 'relationship_guid' => $group_guid, 'inverse_relationship' => TRUE, 'limit' => 9999));
            $area2 .= elgg_view('teams/membershiprequests_list',array('requests' => $list_reqs));
