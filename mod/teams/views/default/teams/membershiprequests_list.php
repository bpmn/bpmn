...
<div class="contentWrapper">
	...
	// Group membership requests
		if ($list_requests) {
			$content = "";
			foreach($list_requests as $team_guid=>$entities) {
				//$num_gr = get_entities_from_relationship('membership_request',$group->guid,true,'','',0,'',9999,0,true);
				//if ($num_gr && $num_gr > 0) {
					//$count += $num_gr;
					// First show the group
                                        $count=count($entities);
                                        $team = get_entity((int)$team_guid);
					$content .= "<div class='clearfloat'></div>";
					$content .= "<table class='request'>\n";
					$content .= "<tr>\n";
					$content .= "<td rowspan='2'>" . elgg_view("teams/icon", array("entity" => $team, "size" => "small")) . "</td>\n";
					$content .= "<td class='name'><a href='" . $team->getURL() . "' title='" . $team->briefdescription . "'>" . $team->name . "</a></td>\n";
					$content .= "</tr>\n";
					$content .= "<tr>\n";
					$content .= "<td>";
					...
					foreach($entities as $entity) {
						...
						$content .= "<tr>\n";
						$content .= "<td>";
						$content .= "<a href='" . $CONFIG->wwwroot . "action/teams/addtogroup?user_guid=" . $entity->guid . "&group_guid=" . $team->guid . "&__elgg_ts=" . $ts ."&__elgg_token=" . $token . "' title='" . elgg_echo("requestnotifications:approve") . "'>" . elgg_echo("requestnotifications:approve") . "</a>";
						$content .= "&nbsp;|&nbsp;";
						$content .= "<a href='" . $CONFIG->wwwroot . "action/teams/killrequest?user_guid=" . $entity->guid . "&group_guid=" . $team->guid . "&__elgg_ts=" . $ts ."&__elgg_token=" . $token . "' title='" . elgg_echo("requestnotifications:decline") . "'>" . elgg_echo("requestnotifications:decline") . "</a>";
						$content .= "</td>\n";
						$content .= "</tr>\n";
						$content .= "</table>\n";
					}
