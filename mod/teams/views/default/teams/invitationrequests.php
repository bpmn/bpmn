		...
		<div class="teams_membershiprequest_buttons">
			<?php
				...
				echo str_replace('<a', '<a class="delete_report_button" ', elgg_view('output/confirmlink',array(
					'href' => $vars['url'] . "action/teams/killinvitation?user_guid={$user->getGUID()}&group_guid={$group->getGUID()}",
					'confirm' => elgg_echo('teams:invite:remove:check'),
					'text' => elgg_echo('delete'),
				)));
			$url = elgg_add_action_tokens_to_url("{$vars['url']}action/teams/join?user_guid={$user->guid}&group_guid={$group->guid}");
			?>
...
<?php
		...
		echo "<p>" . elgg_echo('teams:invitations:none') . "</p>";
