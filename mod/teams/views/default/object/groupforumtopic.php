...
if (get_context() == "search") {
	...
	if (($last_time) && ($u)) $info.= sprintf(elgg_echo('teams:lastupdated'), elgg_view_friendly_time($last_time), " <a href=\"" . $u->getURL() . "\">" . $u->name . "</a>");
	$info .= '</span></p>';
	...
	$info .= "<p>" . elgg_echo('teams:topic') . ": <a href=\"{$vars['entity']->getURL()}\">{$title}</a></p>";
	...
	if ($last_time && $u) {
		...
		$text = sprintf(elgg_echo('teams:lastcomment'), elgg_view_friendly_time($last_time), $commenter_link);
		$info .= "<br /><span class='timestamp'>$text</span>";
	}

	if (teams_can_edit_discussion($vars['entity'], page_owner_entity()->owner_guid)) {
		...
		$info .= "<span class=\"timestamp\">" . elgg_view("output/confirmlink", array(
				'href' => $vars['url'] . "action/teams/deletetopic?topic=" . $vars['entity']->guid . "&group=" . $vars['entity']->container_guid,
	...
	$info .= "<p>" . elgg_echo('teams:started') . " " . $topic_owner->name . ": <a href=\"{$vars['entity']->getURL()}\">{$title}</a></p>";
