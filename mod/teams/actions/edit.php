...
if (($group_guid) && (!$group->canEdit())) {
	register_error(elgg_echo("teams:cantedit"));
...
// Validate create
if (!$group->name) {
	register_error(elgg_echo("teams:notitle"));
...
$group->pages_enable = get_input('pages_enable', 'no');
$group->forum_enable = get_input('forum_enable', 'no');
