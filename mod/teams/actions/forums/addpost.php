...
/**
 * Elgg teams: add post to a topic 
 *
 * @package ElggGroups
 */
...
if (!$group->isMember($user)) {
	register_error(elgg_echo("teams:notmember"));
	forward($_SERVER['HTTP_REFERER']);
}
...
if ($post_id == false) {
	system_message(elgg_echo("teamspost:failure"));
	forward($_SERVER['HTTP_REFERER']);
}
...
system_message(elgg_echo("teamspost:success"));
