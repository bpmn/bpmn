<?php
    ...
    /**
	 * Elgg teams plugin edit topic action.
	 * 
	 * @package ElggGroups
	 */
...
if (!$group_entity->isMember($vars['user']) && !isadminloggedin()) {
	...
	// Make sure we actually have permission to edit
		...
		if ($topic)
		{
			...
			// Make sure the title isn't blank
				if (empty($title) || empty($message)) {
					register_error(elgg_echo("teamstopic:blank"));
			...
			// save the changes
	           ...
	           // Success message
					system_message(elgg_echo("teams:forumtopic:edited"));
		...
		// Forward to the group forum page
	        global $CONFIG;
	        $url = $CONFIG->wwwroot . "pg/teams/forum/{$group_guid}/";
			forward($url);
