...
<form action="<?php echo $CONFIG->wwwroot; ?>mod/upload_users/upload.php" method="POST" enctype="multipart/form-data">
...
echo elgg_view('input/file', array('internalname' => 'csvfile')); 
echo '<br />';

/// Display encoding pull down
$options = array('UTF8', 'ISO-8859-1');
echo '<h4>' . elgg_echo('upload_users:encoding') . '</h4>';
echo elgg_view('input/pulldown', array('options' => $options, 'internalname' => 'encoding'));
echo '<br />';

/// Display delimiter pull down
$options = array(',', ';', ':', '|');
echo '<h4>' . elgg_echo('upload_users:delimiter') . '</h4>';
echo elgg_view('input/pulldown', array('options' => $options, 'internalname' => 'delimiter'));
echo '<br />';
